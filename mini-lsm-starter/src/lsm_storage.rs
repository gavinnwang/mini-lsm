...
impl LsmStorageInner {
    ...
    pub fn get(&self, key: &[u8]) -> Result<Option<Bytes>> {
        ...
        if let Some(data) = state.memtable.get(key) {
            return Ok(if data.is_empty() { None } else { Some(data) });
        }
        let result = state
            .imm_memtables
            .iter()
            .find_map(|mem| mem.get(key))
